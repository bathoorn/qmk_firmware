  ...
  switch (last_char) {
    ...
    case 'l':
      ...
      switch (prev_char) {
        ...
        case 'd':
        case 'y':
          mod_type_num(SHIFT, direction, num); // select n postitions in direction
          if (prev_char == 'y') {
            mod_type(CTRL, KC_C); // copy selection if y
            tap_code(KC_LEFT);
            tap_code(KC_RIGHT);
          }
          tap_code(KC_DEL); // delete selection if c or d
          if (prev_char == 'c')
            go_insert_mode();
          break;
        default:
    ...
    case 'e':
      switch (prev_char) {
        ...
        case 'y':
          HOLD_ALT;
          HOLD_SHIFT;
          tap_code_num(KC_RIGHT, num); // select number of words to right
          ...
          if (prev_char == ':' && last_char == 'w' && num == 1) {
            mod_type(CTRL, KC_S); // save document in most apps
          }
          return true;
    ...
    case '/':
      mod_type(CTRL, KC_F); // search
      go_insert_mode();
      return true;

    case 'u':
      mod_type(CTRL, KC_Z); // undo
      return true;

    case 'o':
      mod_type(CTRL, KC_RIGHT); // goto other side of the selected text
      tap_code(KC_ENT);
      go_insert_mode();
      return true;
    ...
    case 'D':
    case 'C':
      mod_type(SHIFT, KC_END); // select until end of line
    ...
    case 'O':
      tap_code(KC_HOME); // go to the begining of the line
      tap_code(KC_ENT);  // insert newline
      go_insert_mode();
      return true;
    ...
    case 'n':
      tap_code(KC_F3); // search next occurence
      return true;
    ...
    // as these two cases
    case '\\':
      tap_code(KC_END); // goto end of line
      tap_code(KC_ENT); // insert newline
      mod_type(CTRL, KC_V); // copy
      return true;

    case 'q':
      // :q is save
      if (prev_char == ':' && num == 1) {
        mod_type(CTRL, KC_Q);  // quit
      }
      return true;
    ...
    case 'b':
      switch (prev_char) {
        case 'd':
          HOLD_SHIFT;
          mod_type_num(ALT, KC_LEFT, num); // select num words to the left
          UNHOLD_SHIFT;
          tap_code(KC_DEL); // delete
          break;
    ...
    case 'd':
      switch (prev_char) {
        case 'd':
          tap_code(KC_HOME);  // goto the beginning of the line
          mod_type_num(SHIFT, KC_DOWN, num); //select the whole line
          tap_code(KC_DEL);  // delete
          return true;
        case NO_CHAR:
          if(visual_mode) {
    ...
    case 'y':
      switch (prev_char) {
        case 'y':
          tap_code(KC_HOME);  // goto the beginning of the line
          mod_type_num(SHIFT, KC_DOWN, num); // select num lines
          mod_type(CTRL, KC_C);  // copy the selection
          tap_code(KC_RIGHT);
          tap_code(KC_LEFT);
          return true;
        default:
    ...
    case '^':
      if (currcmdsize != 1)
        return false;
      if(visual_mode) HOLD_SHIFT;
      tap_code(KC_HOME);    // select until the beginning of the line
      if(visual_mode) UNHOLD_SHIFT;
      return true;
    ...
    case '$':
      switch (prev_char) {
        ...
        case 'c':
        case 'y':
          mod_type(SHIFT, KC_END); // select until the end of the line
          if (prev_char == 'y') {
            mod_type(CTRL, KC_C); // copy selection
            tap_code(KC_LEFT);
            tap_code(KC_RIGHT);
            tap_code(KC_LEFT);
            visual_mode = false;
        ...
        default:
          if(visual_mode) HOLD_SHIFT;
          tap_code(KC_END); // select until the end of the line
          if(visual_mode) UNHOLD_SHIFT;
          return true;
      }
      return true;
    ...
    case 'G':
      if(prev_char == 'd' || prev_char == 'x' || prev_char == 'X' || prev_char == 'c' || prev_char == 'y') {
        HOLD_SHIFT;
        mod_type(CTRL, KC_END);  // select to the end of the document
        UNHOLD_SHIFT;
    ...
    case 'g':
      if (prev_char == 'g') {
        ...
        if(ch2 == 'd' || ch2 == 'x' || ch2 == 'X' || ch2 == 'c' || ch2 == 'y') {
          HOLD_SHIFT;
          mod_type(CTRL, KC_HOME); // select to the beginning of the document
          UNHOLD_SHIFT;
          if (ch2 == 'c' || ch2 == 'd')
            tap_code(KC_DEL);  // delete selection
          if (ch2 == 'c')
